...
class auth_plugin_ldap extends auth_plugin_base {
    ...
    function cache_userinfo ($conn, $entry) {
        ...
        for ($j=0; $j<$attributes['count']; $j++) {
            ...
            $attrkey = textlib::strtolower($attributes[$j]);
            if (is_array($values)) {
                $user_entry[$attrkey] = $values;
            }
            else {
                $user_entry[$attrkey] = array($values);
            }
        }
        ...
        // Cache in lowercase and as from LDAP
        $this->userinfo_cache[moodle_strtolower($result['username'])] = $result;
    ...
    function sync_users($do_updates=true) {
        ...
        if (!defined('AUTH_LDAP_BULK_INSERT_RECORDS')) {
            $bulk_insert_records = 1000;
        } else {
            $bulk_insert_records = AUTH_LDAP_BULK_INSERT_RECORDS;
        }
        ...
        $orig_count = $DB->get_record_sql("SELECT COUNT(username) as count from {$CFG->prefix}user u where auth='ldap'");
        $orig_count = $orig_count->{'count'};

        $sitename = $DB->get_field('course', 'shortname', array('id' => SITEID));

    /// Define table user to be created
        $temptable = $CFG->prefix . "tmp_extuser";
        ...
        foreach ($contexts as $context) {
            ...
            do {
                $pagecount = 0;
                if (!empty($this->config->search_paging) && $this->config->search_paging) {
                    ldap_control_paged_result($ldapconnection, PAGE_SIZE, true, $cookie);
                }
                ...
                if (!empty($this->config->search_paging) && $this->config->search_paging) {
                    ldap_control_paged_result_response($ldapconnection, $ldap_result, $cookie);
                }
                unset($ldap_result); // free mem
            } while ($cookie !== null  && $cookie !== '' && (!empty($this->config->search_paging) && $this->config->search_paging));
            $transaction->allow_commit();
            if (!empty($this->config->search_paging) && $this->config->search_paging) {
                print "\npagecount ended up as $pagecount\n";
            }
            $pagecount = 0;
        }
        ...
        print "Have $orig_count LDAP auth records in Moodle\n\n";
        $processremoveuser = true;
        if (($count < $orig_count * 0.98) && ($this->config->removeuser != AUTH_REMOVEUSER_KEEP)) {
            $processremoveuser = false;
            print "Skipping removal of users. Broke the '2% rule'.\n\n";
            $admin = get_admin();
            $admin->email = 'moodle.techsupport@netspot.com.au';
            $admin->emailstop = false;
            $subject = "LDAP Sync alert: $sitename";
            $message = "Skipping removal of users. Broke the '2% rule'.\n\n";
            $message .= "Moodle Users: $orig_count\nLDAP Users: $count.\n";
...
/// User removal
        ...
        if ($this->config->removeuser != AUTH_REMOVEUSER_KEEP && $processremoveuser) {
            $sql = 'SELECT u.*
                      FROM {user} u
...
/// User Additions
    ...
    function ldap_bulk_insert($users, $temptable) {
        ...
        foreach ($users as $u) {
            ...
            // join sql
            $sql = $sql . "('$u',{$CFG->mnet_localhost_id})";
       #     print "\t+ " . count($users) . " users\n";
            if (!($icount % 1000)) { print "$icount .. "; }
            $i++;
            $icount++;
        }
        $DB->execute($sql);
    ...
    function process_config($config) {
        ...
        if (!isset($config->search_paging)) {
             $config->search_paging = 0;
        }
        ...
        set_config('search_paging', $config->search_paging, $this->pluginconfig);
